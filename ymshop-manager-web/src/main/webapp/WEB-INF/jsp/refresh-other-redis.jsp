...
<div style="margin-left: 1vw;margin-right: 1vw" class="cl pd-5 bg-1 bk-gray mt-20">
    <span class="l">
        ...
        <a class="btn btn-primary radius" onclick="refreshThankRedis()" href="javascript:;"><i class="Hui-iconfont">&#xe6bd;</i> 刷新支付板块缓存</a>
    </span>
</div>
<table class="table">
    <div class="page-container">
        <form action="" method="post" class="form form-horizontal" id="content-cat-edit">
            ...
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-1">支付板块缓存内容：</label>
                <div class="formControls col-xs-6 col-sm-6">
                    <textarea name="thank" id="thank" class="textarea"  placeholder="当前支付板块缓存已是最新" style="height: 250px;margin-bottom: 1vw"></textarea>
                </div>
            </div>
        </form>
    </div>
...
<script type="text/javascript">
    ...
    function refreshThankRedis() {
        layer.confirm('确认要刷新支付板块缓存吗？',{icon:0},function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                ...
                success: function(data) {
                    ...
                    if(data.success==true){
                        getThankRedisData();
                        msgSuccess("刷新成功!重新打开支付板块即可重新存入新缓存");
                    }else{
                        layer.alert('刷新失败! '+data.message, {title: '错误信息',icon: 2});
                    }
                },
